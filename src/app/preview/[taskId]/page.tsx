...
export default function PreviewPage() {
  ...
  const poll = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        setError((data as { error?: string }).error || "ステータス取得に失敗しました");
        return false;
      }
  ...
  const progress = isSucceeded
    ? 100
    : isFailed
    ? 0
    : Math.min(task?.progress ?? 5, 95);

  const statusText = (() => {
    if (!task) return "タスクを確認中...";
    if (task.status === "PENDING") return "キューに追加されました";
    if (task.status === "IN_PROGRESS") return `AI処理中 (${task.progress ?? 0}%)`;
    if (task.status === "SUCCEEDED") return "3Dモデルの生成が完了しました";
    if (task.status === "FAILED") return task.error || "生成に失敗しました";
    if (task.status === "EXPIRED") return "タスクの有効期限が切れました";
    return "";
  })();
    ...
    <div className="min-h-dvh flex flex-col bg-[#0a0a0f]">
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="relative flex-1 min-h-[50vh] bg-[#0d0d18]">
          {isSucceeded && glbUrl ? (
            <ModelViewer glbUrl={glbUrl} className="w-full h-full min-h-[50vh]" />
          ) : (
            <div className="flex items-center justify-center h-full min-h-[50vh]">
              {isFailed ? (
                <div className="text-center p-8">
                  ...
                  <p className="text-white/40 text-sm mt-2">{task?.error}</p>
                </div>
              ) : (
                <div className="text-center p-8">
        ...
        <div className="p-4 space-y-4">
          {!isSucceeded && (
            <ProgressTracker
              progress={progress}
              status={statusText}
              stage={isFailed ? "error" : "processing"}
            />
          )}
          ...
          {isSucceeded && task?.model_urls && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {task.model_urls.glb && (
                  <button
                    onClick={() => handleDownload(task.model_urls!.glb!, "glb")}
                ...
                {task.model_urls.stl && (
                  <button
                    onClick={() => handleDownload(task.model_urls!.stl!, "stl")}
                    className="glass rounded-xl p-3 text-left hover:bg-white/10 transition-colors"
                  >
                ...
                {task.model_urls.obj && (
                  <button
                    onClick={() => handleDownload(task.model_urls!.obj!, "obj")}
                    className="glass rounded-xl p-3 text-left hover:bg-white/10 transition-colors"
                  >
                ...
                {task.model_urls.fbx && (
                  <button
                    onClick={() => handleDownload(task.model_urls!.fbx!, "fbx")}
                    className="glass rounded-xl p-3 text-left hover:bg-white/10 transition-colors"
                  >
            ...
            <div className="glass rounded-2xl p-4 space-y-2">
              ...
              <ul className="space-y-2 text-sm text-white/60">
                <li className="flex gap-2">
                  ...
                  <span>STLファイルをダウンロードしてください</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-purple-400 shrink-0">2.</span>
                  <span>3Dプリント業者（DMM.make, Shapeways等）のサイトへアクセス</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-purple-400 shrink-0">3.</span>
                  <span>STLファイルをアップロードして素材・サイズを選択</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-purple-400 shrink-0">4.</span>
                  <span>注文完了！数日〜1週間で届きます</span>
                </li>
              </ul>
            </div>
          )}
