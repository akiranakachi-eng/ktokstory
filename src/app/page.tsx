    ...
    <div className="min-h-dvh flex flex-col bg-[#0a0a0f]">
      ...
      <header className="px-4 pt-6 pb-4 text-center">
        <div className="inline-flex items-center gap-2 glass px-4 py-2 rounded-full mb-4">
          ...
          <span className="text-white/60 text-xs">AI powered by Meshy</span>
        </div>
        <h1 className="text-3xl font-bold gradient-text mb-2">
          3D Print Studio
        </h1>
        <p className="text-white/50 text-sm max-w-xs mx-auto">
          写真1枚をAIが3Dモデルに変換。そのまま業者に入稿できます。
        </p>
      </header>
      ...
      <main className="flex-1 px-4 pb-8 max-w-lg mx-auto w-full space-y-4">
        ...
        <section className="mt-6 space-y-3">
          <h2 className="text-white/40 text-xs font-semibold uppercase tracking-wider text-center">
            使い方
          </h2>
        ...
        <section className="glass rounded-2xl p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {["DMM.make", "Shapeways", "i.materialise", "Sculpteo", "JLC3DP"].map((s) => (
              <span key={s} className="text-xs bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-white/60">
                {s}
              </span>
            ))}
          </div>
