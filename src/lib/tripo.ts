...
export interface TripoTask {
  task_id: string; type: string; status: TripoTaskStatus; progress: number;
  result?: { pbr_model?: { glb?: string; fbx?: string; obj?: string; stl?: string; usdz?: string; }; rendered_image?: { url?: string }; };
...
export async function uploadImage(fileBuffer: Buffer, mimeType: string): Promise<string> {
  ...
  const res = await fetch(`${TRIPO_API_BASE}/upload`, { method: "POST", headers: { Authorization: `Bearer ${apiKey}` }, body: form });
...
export async function createImageTo3DTask(fileToken: string, mimeType: string): Promise<string> {
  ...
  const res = await fetch(`${TRIPO_API_BASE}/task`, {
    ...
    body: JSON.stringify({ type: "image_to_model", file: { type: fileType, file_token: fileToken } }),
  });
  if (!res.ok) throw new Error(`Tripo task error ${res.status}: ${await res.text()}`);
  const json = await res.json();
...
export async function getTaskStatus(taskId: string): Promise<TripoTask> {
  const apiKey = process.env.TRIPO_API_KEY;
  if (!apiKey) throw new Error("TRIPO_API_KEY is not set");
  const res = await fetch(`${TRIPO_API_BASE}/task/${taskId}`, { headers: { Authorization: `Bearer ${apiKey}` }, cache: "no-store" });
  if (!res.ok) throw new Error(`Tripo status error ${res.status}: ${await res.text()}`);
  const json = await res.json();
