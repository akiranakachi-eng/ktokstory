...
export function heightmapToSTL(
  ...
  const scaleXY = 100; // mm
  const maxRelief = 20; // mm, max height above base
  const base = 3; // mm, solid base thickness

  const w = width;
  const h = height;
  const topTris = (w - 1) * (h - 1) * 2;
  const bottomTris = (w - 1) * (h - 1) * 2;
  // 2 triangles per edge segment x 4 walls
  ...
  // Top surface - winding gives +Z normal
  for (let j = 0; j < h - 1; j++) {
    for (let i = 0; i < w - 1; i++) {
  ...
  // Bottom surface (z=0) - reversed winding gives -Z normal
  for (let j = 0; j < h - 1; j++) {
    for (let i = 0; i < w - 1; i++) {
